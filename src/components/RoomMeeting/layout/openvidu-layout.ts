...
class OpenViduLayout {
  ...
  positionElement(elem: HTMLElement, x: number, y: number, width: number, height: number, animate: any) {
  ...
  getBestDimensions(minR: number, maxR: number, count: number, WIDTH: number, HEIGHT: number) {
		let maxArea: number = 0;
		let targetCols: number = 0;
		let targetRows: number = 0;
  ...
  arrange(
    ...
    if (!fixedRatio) {
      dimensions = this.getBestDimensions(minRatio, maxRatio, count, WIDTH, HEIGHT);
    } else {
      const ratio = this.getVideoRatio(children.length > 0 ? children[0] : null);
      dimensions = this.getBestDimensions(ratio, ratio, count, WIDTH, HEIGHT);
    }
  ...
  filterDisplayNone(element: HTMLElement) {
  ...
  initLayoutContainer(container: HTMLElement | string, opts: OpenViduLayoutOptions) {
    this.opts = {
    ...
    this.layoutContainer = typeof container === 'string' ? $(container)[0] : container;
  }
