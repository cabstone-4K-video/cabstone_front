...
interface LayoutOptions {
  ...
  minRatio?: number;
  fixedRatio?: boolean;
  animate?: { duration?: number; easing?: string; complete?: () => void } | boolean;
  bigClass?: string;
...
class OpenViduLayout {
  ...
  fixAspectRatio(elem: HTMLElement, width: number) {
  ...
  positionElement(elem: HTMLElement, x: number, y: number, width: number, height: number, animate?: { duration?: number; easing?: string; complete?: () => void } | boolean) {
    ...
    if (animate && $) {
      $(elem).stop();
      $(elem).animate(targetPosition, (animate as { duration?: number; easing?: string }).duration || 200, (animate as { easing?: string }).easing || 'swing', () => {
        this.fixAspectRatio(elem, width);
        if ((animate as { complete?: () => void }).complete) {
          (animate as { complete?: () => void }).complete!.call(this);
        }
      });
    } else {
  ...
  getBestDimensions(minR: number, maxR: number, count: number, WIDTH: number, HEIGHT: number, targetHeight: number) {
    let maxArea: number | undefined, targetCols: number | undefined, targetRows: number | undefined, targetWidth: number | undefined;
    let tWidth, tHeight, tRatio;
  ...
  arrange(
    ...
    let x = 0,
      y = 0;
    const rows: { children: HTMLElement[]; width: number; height: number }[] = [];

    for (let i = 0; i < children.length; i++) {
      let row: { children: HTMLElement[]; width: number; height: number };
      ...
      let targetWidth = dimensions.targetWidth || 0; // 기본값 0 설정
      targetHeight = dimensions.targetHeight || 0; // 기본값 0 설정
      if (fixedRatio) {
  ...
  initLayoutContainer(container: HTMLElement | string, opts: LayoutOptions) {
    this.opts = {
    ...
    this.layoutContainer = typeof container === 'string' ? document.querySelector(container) as HTMLElement : container;
  }
